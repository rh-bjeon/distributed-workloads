...
# ---- 변경: 명시적 S3 URI를 받을 수 있도록 인자 추가 ----
def get_pretrained_path(model_id: str, bucket_uri: Optional[str] = None):
    """
    지정된 bucket_uri가 있으면 그 위치에서 refs/main을 읽어
    HF 캐시 경로를 반환합니다. 없으면 기존 미러 규칙(get_mirror_link) 사용.
    """
    mirror_uri = bucket_uri or get_mirror_link(model_id)
...
def get_tokenizer(model_name, **kwargs):
    pretrained_path = get_pretrained_path(model_name, bucket_uri=kwargs.get("model_s3_uri"))
...
def _test_tokenizer(model_name, model_s3_uri=None):
    tokenizer = get_tokenizer(
        model_name=model_name, special_tokens=["<REPR_END>"], model_s3_uri=model_s3_uri
    )
...
def checkpoint_model(
    ...
    print(f"checkpointing: checkpoint_folder={checkpoint_folder}, ckpt_id={ckpt_id}")
...
def training_function(kwargs: dict):
    ...
    # ---- 변경: 명시 S3 경로 우선 사용 ----
    explicit_bucket_uri = args.model_s3_uri  # may be None
    bucket_uri = explicit_bucket_uri or get_mirror_link(model_id)

    # 동일 노드 다중 프로세스 동시 다운로드 방지
    ...
    _test_tokenizer(args.model_name, model_s3_uri=explicit_bucket_uri)
    tokenizer = get_tokenizer(
        model_name=args.model_name,
        chat_template=chat_template,
        special_tokens=special_tokens,
        model_s3_uri=explicit_bucket_uri,
    )
    ...
    pretrained_path = get_pretrained_path(model_id, bucket_uri=bucket_uri)
...
def parse_args():
    ...
    # ---- 새 인자: 모델이 있는 S3 경로 직접 지정 ----
    parser.add_argument(
        "--model-s3-uri",
        type=str,
        default=None,
        help=(
            "학습에 사용할 모델이 저장된 S3 디렉터리 URI. "
            "예: s3://my-bucket/models--meta-llama--Meta-Llama-3.1-8B "
            "(refs/main 및 snapshots/<hash> 구조를 포함해야 함)"
        ),
    )
...
def main():
    ...
    trainer = TorchTrainer(
        ...
        run_config=train.RunConfig(
            storage_path=urljoin(args.storage_path, args.model_name) if args.storage_path else None,
